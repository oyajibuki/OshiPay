...
def _render_discord_button(url: str, label: str = "Discordアカウントで登録 / ログイン"):
    # :not(:first-of-type) = 2番目以降のリンクボタンだけblurpleに適用
    # 1番目（Google）は白背景のまま。Discordボタンが単独の場合も考慮不要（常にGoogle→Discordの順）
    st.markdown(f"""
    <style>
    [data-testid="stLinkButton"]:not(:first-of-type) > a {{
    ...
    [data-testid="stLinkButton"]:not(:first-of-type) > a:hover {{
